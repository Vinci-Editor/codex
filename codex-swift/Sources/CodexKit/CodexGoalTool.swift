        ...
        case .update:
            return CodexJSONSchema.object(
                properties: [
                    "status": .stringEnum(
                        ["complete", "blocked"],
                        description: "Required. Set to complete only when the objective is achieved and no required work remains. Set to blocked only when the goal cannot currently proceed without a user decision, missing dependency, or external unblock."
                    ),
                ],
                required: ["status"]
            ).inputSchema
